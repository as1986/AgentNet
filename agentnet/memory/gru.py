...
class GRUNLMemoryLayer(lasagne.layers.MergeLayer):
    """
    ...
    def __init__(self,
                 ...
                 nl_update_type=lasagne.nonlinearities.relu,
                 grad_clipping=5.,
                 **kwargs):
        ...
        super(GRUNLMemoryLayer, self).__init__([prev_state_input, observation_input], **kwargs)
        self.grad_clipping = grad_clipping
        ...
        self.nl_update = self.add_param(nl_update, (num_units, num_units),
                                        name='nl_update')
        self.nl_update_b = self.add_param(nl_update_b, (num_units,),
                                          name='nl_update_b')
        self.nl_update_type = nl_update_type

        # Stack input weight matrices into a (num_inputs, 3*num_units)
        # matrix, which speeds up computation
        self.W_in_stacked = T.concatenate(
            [self.W_in_to_resetgate, self.W_in_to_updategate,
             self.W_in_to_hidden_update, self.W_in_to_hidden_update_2], axis=1)

        # Same for hidden weight matrices
        self.W_hid_stacked = T.concatenate(
            [self.W_hid_to_resetgate, self.W_hid_to_updategate,
             self.W_hid_to_hidden_update, self.W_hid_to_hidden_update_2], axis=1)

        # Stack gate biases into a (3*num_units) vector
        self.b_stacked = T.concatenate(
            [self.b_resetgate, self.b_updategate,
             self.b_hidden_update, self.b_hidden_update_2], axis=0)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        # Compute (1 - u_t)h_{t - 1} + u_t c_t
        pre_nl_update = update_gate * hidden_update
        nl_update = self.nl_update_type(self.nl_update * pre_nl_update + self.nl_update_b)
        hid = (1 - update_gate) * last_memory_state + nl_update
